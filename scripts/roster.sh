...
cmd_lock() {
  local dir f base; dir="$(agents_dir)"; f="$(resolve_file "$dir" "$1")"
...
cmd_unlock() {
  local dir name f base stem; dir="$(agents_dir)"; name="${1%.md}"
  while IFS= read -r f; do
    base="$(basename "$f")"; stem="${base#locked_}"; stem="${stem%.md}"
...
cmd_disable() {
  local dir f; dir="$(agents_dir)"; f="$(resolve_file "$dir" "$1")"
...
cmd_enable() {
  local dir name f base stem; dir="$(agents_dir)"; name="${1%.md}"
  while IFS= read -r f; do
    base="$(basename "$f")"; stem="${base%.md.off}"
    if [[ "$stem" == "$name" || "$(fm_field "$f" name)" == "$name" ]]; then
      mv -n -- "$f" "${f%.off}"
...
case "${1:-}" in
  list)    cmd_list ;;
  lock)    [[ $# -ge 2 ]] || err "usage: roster.sh lock <name>";    cmd_lock "$2" ;;
  unlock)  [[ $# -ge 2 ]] || err "usage: roster.sh unlock <name>";  cmd_unlock "$2" ;;
  disable) [[ $# -ge 2 ]] || err "usage: roster.sh disable <name>"; cmd_disable "$2" ;;
  enable)  [[ $# -ge 2 ]] || err "usage: roster.sh enable <name>";  cmd_enable "$2" ;;
